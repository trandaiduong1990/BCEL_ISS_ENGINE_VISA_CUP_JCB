...
public class BalanceRequest extends RequestBaseHandler {
	...
	public IParser execute(IParser objISO)throws TPlusException {
		...
		try
		{
			...
			System.out.println("objISO.hasField(4) :: " + objISO.hasField(4));

			/*if(!objISO.hasField(4)){
				objISO.setValue(4, "000000000000");
			}
			objISO.setValue(51, "418");*/
			...
			if(objRes!=null)
			{
				...
				objRes.dump(System.out, "");

				if(!objISO.hasField(54)){

					if(objISO.hasField(39))
					{
						if("00".equals(objISO.getValue(39)))
						{
							objISO.setValue(TPlusISOCode.RESPONSE_CODE,"05");
							objTranxBean.setResponseCode("05");
							objTranxBean.setRemarks("No F54 on CB response. but RC 00");
						}
