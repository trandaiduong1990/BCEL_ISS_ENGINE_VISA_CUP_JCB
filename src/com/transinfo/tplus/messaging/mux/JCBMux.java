...
public class JCBMux implements IMux  {
	...
	public static JCBMux getJCBMux(String name){
		System.out.println("in getJCBMux");
		JCBMux mux;
		String key = JCBMux.class.getName() + "." + name;
		try {
			mux = (JCBMux)NameRegistrar.get(key);
	...
	public ISOMsg process(ISOMsg isomsg){
		...
		try
		{
			...
			if (DebugWriter.boolDebugEnabled) DebugWriter.write("VisaMux:VISA Connected.."+jcbMux.isConnected());
			
			/*ISOPackager packager = new JCBPackager();
			...
			System.out.println("hexdump requestMsg.pack() " + ISOUtil.hexdump(requestMsg.pack()));
			System.out.println("");
			System.out.println("hexString requestMsg.pack() " + ISOUtil.hexString(requestMsg.pack()));

			channel.setHeader(ISOUtil.hex2byte(HeaderUtil.getJCBReqHeader(requestMsg.pack().length)));

			jcbMux.queue(request);
			...
			try {
				...
				System.out.println("In Visa Mux"+ignore);
			}
	...
	public void initialize(IParser objISO){
		...
		//channel = new ASCIIChannel(visaHost, visaPort, packager);
		//channel = new JCBChannel(visaHost, visaPort, packager);
		//channel = new RawChannel(visaHost, visaPort, packager, "0000".getBytes());
		//channel = new TestChannel(visaHost, visaPort, packager, "0000".getBytes());
		channel = new CSChannel(visaHost, visaPort, new JCBPackager());

		jcbMux = new ISOMUX(channel);
		jcbMux.setISORequestListener(new JCBRequestListener());
		new Thread(jcbMux).start();
		
		/*String key = JCBMux.class.getName() + "." + "JCB";
		NameRegistrar.register(key, jcbMux);*/		
		
		// commented only for testing
		if(!INITIALIZED)
		{
			Thread t = new Thread(new EchoManager());
			t.start();
		}
		...
		channel.setLogger(logger,"CACIS-JCB");
		...
		try {
			...
			FileOutputStream fwDebug = new FileOutputStream("E:\\Projects\\BCEL\\Issusing\\IssusingEngine\\DEV22\\ISOLog.txt",true);
